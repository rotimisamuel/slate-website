  ...
  if ($.exists('.swiper-container')) {
    var swiper = new Swiper('.swiper-container', {
      ...
      autoplay: {
        ...
        // pauseOnMouseEnter: true  // (optional if your Swiper supports it)
      },
      pagination: {
        el: '.swiper-pagination',
        clickable: true,
    ...
    // Correct the fraction numbers when loop duplicates slides
    var totalSlides = swiper.slides.length - (swiper.loopedSlides * 2 || 0);
    $('.slider-fraction span:first-child').text('1');
    $('.slider-fraction span:last-child').text(totalSlides);

    swiper.on('slideChange', function () {
      $('.slider-fraction span:first-child').text(swiper.realIndex + 1);
    });
  }
}
